...
let invertedIndexTable = new Map(),
    childTable = [],
    descendantsTable = [],
    addressMappingTable = []
                ...
                let readFile = await dfd.read_excel(process.cwd()+"/public/cpc/"+local_xcel).then((readFile)=>{
                    ...
                    structRecordTree(cpcItems).then(()=>{
                        ...
                        setNumberDescendants(rootRecord).then(()=>{
                            ...
                            structInvertedIndexTable(rootRecord).then(()=>{
                                console.log("4.structInvertedIndexTable")
                                // console.log(invertedIndexTable)
                                console.log(invertedIndexTable.length)
                                structChildTable(rootRecord).then(()=>{
                                    ...
                                    structAddressMappingTable(rootRecord).then(()=>{
                                        ...
                                        structDescendantsTable(rootRecord).then(()=>{
                                            ...
                                            cutChildInInvertedIndexTable().then(()=>{
                                                console.log("8.cutChildInInvertedIndexTable")
                                                // console.log(invertedIndexTable)
                                                // console.log("TOOLS",invertedIndexTable.get("TOOLS"))
...
let intersectionRecordsSearch = async (keywordToken) => {
    let searchableToken = []
        intersectionRecords = [],
        getKeywordIndex = [],
        meetFirst = false

    for(let keyword of keywordToken)
        if(invertedIndexTable.has(keyword))
            searchableToken.push(keyword)
    // invertedIndexTable.get(keyword)은 keyword에 index가 존재하지않으면,
    // 즉 검색어가 cpc 문서에 존재하지 않으면 pass 한다. 
    // 그리고 filter함수를 통해 교집합을 찾을 때
    // intersectionRecords가 빈 배열이면 교집합도 빈 배열이므로 keyword 순서 중 가장 처음으로
    // cpc 문서에 존재하는 검색어에 해당될 경우 intersectionRecords에 저장한다.
    for(let keyword of searchableToken){
        if(!meetFirst){
            intersectionRecords = invertedIndexTable.get(keyword)
            meetFirst = true
            continue
        }
        let num = 0
        getKeywordIndex = invertedIndexTable.get(keyword)
        for(let i = 0; i < intersectionRecords.length; i++){
            for(let j=0; j < getKeywordIndex.length; j++ ){
                if(intersectionRecords[i]==getKeywordIndex[j])
                {       
                    num++
                    console.log(intersectionRecords[i])
                }
            }
        }
        
        // console.log(getKeywordIndex)
        intersectionRecords = intersectionRecords.filter(it => getKeywordIndex.includes(it))
        console.log(intersectionRecords)
        console.log(num)
    }
...
let searchRecords = async (body) => {
    let keywordToken = await tokenizeKeyword(body.keyword)
    console.log(keywordToken)
    let searchTargetRecords = []
    
    if(!(keywordToken.length == 0 || keywordToken[0] == '') )
        searchTargetRecords = await intersectionRecordsSearch(keywordToken)

    // console.log(searchTargetRecords)
